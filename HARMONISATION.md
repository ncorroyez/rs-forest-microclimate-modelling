> Miroir — éditer la version NC_Full

# HARMONISATION des deux dépôts de thèse — document unique (MAJ 2026-09-14)

**Source unique = `NC_Full/HARMONISATION.md`** (`~/Documents/NC_Full/HARMONISATION.md`). Cette copie est
un miroir : ne pas l'éditer, recopier depuis NC_Full. Remplace `HARMONISATION_carto_2026-09-14.md` et
`ARBORESCENCE_CIBLE_2026-09-14.md` (les deux, dans les deux dépôts), archivés dans
`_archive_harmonisation_2026-09-14/` de chaque dépôt.

**Dans ce dépôt (rmusica)** : Ch1 (LiDAR × MuSICA), intro et discussion générales, Annexe G, moteur MuSICA.
Ch1 dans `chapter1/{manuscript,producers,orchestration,ledger,_reference}/`, cadre de thèse dans
`thesis_frame/`, bib de compile `Bib_these.bib` à la racine ; **`R/`, `pipeline/`, `in_files/` restent à la
racine.** L'autre dépôt (Ch2, Ch3, Ch4/GEDI, plans FR, bib `Bib_nathan_corroyez_10Sep.bib`) est
`~/Documents/NC_Full` (« NC_Full »).
