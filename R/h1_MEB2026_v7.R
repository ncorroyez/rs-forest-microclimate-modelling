# ==============================================================================
# MEB 2026 v7 :
#   (1) fig_archetypes_profiles : annotations larger, more to the left,
#       fCover floored at 0.5, H_max with max subscript
#   (2) Other figures : H_max subscript wherever Hmax appears
# ==============================================================================
...
# Display labels for Variables (parsable plotmath expressions)
.VAR_LABEL_PARSE <- c(LAI = "LAI", fCover = "fCover",
                        Hmax = "H[max]", LAD = "LAD")
...
# ============================================================================
# (1) Archetypes profile figure : LAD profile + 3-line stats per cluster
# ============================================================================
build_archetype_profiles <- function() {
  cli_h1("Archetype profiles v7")
  ...
  # FLOOR fCover at 0.5 (per user request)
  ...
  # Trait stats per cluster (with floored fCover)
  stats <- df_sample[, .(LAI_mean = mean(LAI, na.rm=TRUE),
                            ...
                            fCover_sd   = sd(fCover_floored, na.rm=TRUE)),
                        by = Cluster]
  stats[, Cluster := paste0("C", Cluster)]
  stats[, Cluster := factor(Cluster, levels = paste0("C", 1:4))]
  setorder(stats, Cluster)
  cli_alert("Cluster stats (fCover floored at 0.5) :"); print(stats)

  # Build plotmath labels for 3 lines per cluster
  stats[, lab_LAI    := sprintf("LAI == %.2f %%+-%% %.2f",
                                   LAI_mean, LAI_sd)]
  ...
  # Place annotations on the LEFT (smaller x) and HIGHER on Y, LARGER font
  # Use 3 separate geom_text calls (one per line) so that the H_max subscript renders
  x_ann <- max_lad * 0.30   # was 0.55, now closer to left
  p <- ggplot(prof_long, aes(x = lad_mean, y = height,
    ...
    # 3 stacked annotations per cluster
...
# ============================================================================
# (2) Type B with H_max subscript on col title
# ============================================================================
...
# Helper : parsable variable label
.var_lab_expr <- function(v) {
  switch(v,
    "LAI"    = "LAI",
    "fCover" = "fCover",
    "Hmax"   = "H[max]",
...
make_typeB_row <- function(method, x_bounds, y_lim,
  ...
  scatter <- function(v) {
    ...
    ggplot(sub, aes(x = trait, y = delta)) +
      ...
      labs(x = if (show_x_axis) parse(text = .var_lab_expr(v)) else NULL,
            y = if (v == "LAI") method_yexpr(method) else NULL,
            title = if (show_col_titles) parse(text = .var_lab_expr(v)) else NULL) +
      theme_bw(base_size = 15) +
...
build_typeB <- function() {
  cli_h1("Type B v7 (H_max subscript)")
  bds <- compute_bounds()
  rows <- list()
  for (m in c("LOVB","LVA","Shapley")) {
    yb <- bds$y[method == m]
...
# ============================================================================
# (3) HOBO with H_max subscript in facet strip (via label_parsed)
# ============================================================================
...
build_hobo <- function() {
  cli_h1("HOBO v7 (H_max subscript)")
  ...
  # Use parsable labels for Variable so that H[max] renders with subscript
  long[, Variable_lab := .VAR_LABEL_PARSE[Variable]]
...
# ============================================================================
# (4) Heatmap with H_max subscript on y axis label
# ============================================================================
build_heatmap <- function() {
  cli_h1("Heatmap v7 (H_max subscript)")
  ...
  labels  <- c("LOO","LVA","Shapley","LOO","LVA","Shapley")
  ...
  # Parsable y labels (H[max] renders with subscript)
  long_full[, var_lab := .VAR_LABEL_PARSE[as.character(variable)]]
  long_full[, var_lab := factor(var_lab,
                                    levels = c("LAI","fCover","H[max]","LAD"))]
  ...
  p <- ggplot(long_full, aes(x = col_idx, y = var_lab, fill = fill_rank)) +
...
if (sys.nframe() == 0) {
  ...
  build_hobo()
  build_heatmap()
  cli_alert_success("v7 done : archetype profiles refactored + H_max subscript")
}
