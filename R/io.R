...
load_lidar_rasters <- function(in_dir, agg_factor = 2) {
  ...
  # The `lai_z1_res_10_m.tif` / `lad_profiles_z1_res_10_m.tif` rasters are lidR
  # `LAD()` Plant Area Density (PAD). The one-sided->two-sided doubling MuSICA
  # needs is applied ONCE, inside run_musica_one (`lai <- 2 * lai`). We therefore
  # keep the raster at PAD scale here (no ×2) — applying ×2 both here and in
  # run_musica_one double-counted the leaf area (~2× the real canopy vs field LAI).
  # (raster kept at PAD scale; single doubling lives in the MuSICA input.)
...
extract_macro_daily <- function(forcing_file, date_seq) {
  ...
  # Return a variable, or an all-NA series of the right length if the forcing lacks it.
  get_var <- function(v) {
