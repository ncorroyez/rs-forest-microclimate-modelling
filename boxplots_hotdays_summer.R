...
WIN <- as.Date(c("2021-06-01","2021-09-30")); ds_all <- seq(WIN[1], WIN[2], by="day")
...
fwrite(D, "/home/corroyez/Documents/NC_Full/manuscripts/ch3/tables/Table4r_hotdays_summer_boxplot_data.csv")
...
mk <- function(met, ylab, href){
  ggplot(D[metric==met], aes(sx, value, fill=typ)) +
    ...
    labs(x=NULL, y=ylab, subtitle=sprintf("10%% hottest days of summer (macro Tmax ≥ %.1f °C, n = %d days)", thr, length(hot))) +
...
ggsave_article(file.path(outdir,"Fig_boxplot_hotdays_summer_dTmax"), mk("dTmax","ΔTmax (sub-canopy − macro, °C)", 0), w, 4.5)
ggsave_article(file.path(outdir,"Fig_boxplot_hotdays_summer_slope"), mk("slope","slope-and-equilibrium slope", 1), w, 4.5)
