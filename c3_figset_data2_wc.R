...
FMACRO<-"in_files/musica_in_Blois_pblh.nc"  # macro MUST match the pblh forcing the windcorr runs used, NOT CFG_C3$forcing_file (FR-Blo) — pipeline audit
...
sets<-list("S2 only"=c("S2opt","S2atbd"),"FORMS-H only"=c("FORMS_H","hfrac"),"S2 + FORMS-H"=c("S2opt","S2atbd","FORMS_H","hfrac"),"S2 + LiDAR structure"=c("S2opt","S2atbd","Hmax","fCover","VCI","LCV"),"LiDAR structure"=c("Hmax","fCover","VCI","LCV"))
