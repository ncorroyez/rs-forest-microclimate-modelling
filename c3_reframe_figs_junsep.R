# ==============================================================================
# c3_reframe_figs_junsep.R  — Chapter 3 reframed figures, June-September window.
...
# Out: NC_Full/manuscripts/ch3/figures/Fig{1,2,3,4}_reframe_junsep.png
# ==============================================================================
...
Zdir <- "/home/corroyez/Documents/z_Example_rmusica_31012025/out_files/Chapter3/tables"
...
PP  <- fread(file.path(Zdir,"junsep_full_perplot.csv"))
...
LAI <- unique(fread(file.path(Zdir,"annual_notmasked_report.csv"))[,.(id_plot,LAI=LAI_ALS)])
FRT <- fread(file.path(TAB,"reframe_dopt_mechanism.csv"))[,.(id_plot,fr_top)]
BY  <- fread(file.path(Zdir,"junsep_full_by_archetype.csv"))[window=="junsep"]
CON <- fread(file.path(Zdir,"junsep_full_paired_contrasts.csv"))[window=="junsep"]
...
ggsave(file.path(FIG,"Fig1_reframe_junsep.png"), g1, width = 9.5, height = 4.6, dpi = 300, bg = "white")
...
g2 <- p2("slope","Δ coupling slope vs LiDAR") + p2("dtmax","Δ ΔT_max vs LiDAR (°C)") +
  plot_layout(guides="collect") & theme(legend.position="top")
ggsave(file.path(FIG,"Fig2_reframe_junsep.png"),g2,width=9,height=4.4,dpi=300,bg="white")
...
g3 <- p3("slope","Ranking r, coupling slope") + p3("dtmax","Ranking r, ΔT_max") +
  plot_layout(guides="collect") & theme(legend.position="top")
ggsave(file.path(FIG,"Fig3_reframe_junsep.png"),g3,width=9.5,height=4.4,dpi=300,bg="white")
...
ggsave(file.path(FIG,"Fig4_reframe_junsep.png"),g4,width=6,height=4.6,dpi=300,bg="white")
