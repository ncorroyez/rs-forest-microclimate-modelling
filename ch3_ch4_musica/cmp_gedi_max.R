# ==============================================================================
# cmp_gedi_max.R — Chapter 4, "et l'été ?": month-by-month summer
...
# Writes NC_Full/manuscripts/ch4/tables/Table18c_c4_gedimax.csv (+ _boot).
#   Rscript cmp_gedi_max.R
# ==============================================================================
...
SC<-c("DYN_ALS_GEDIMAX","DYN_ALS_GEDIFALL","DYN_S2_ANNUAL_FIX","CONST_ALS","STATIC_ALS")
...
for(w in names(WINDOWS)){
  ...
  boot[[w]]<-rbind(
    cbind(window=w,contrast="GEDIMAX - GEDIFALL (flatten core)",
          pboot(PP[["DYN_ALS_GEDIMAX"]],PP[["DYN_ALS_GEDIFALL"]])),
    cbind(window=w,contrast="GEDIMAX - ANNUAL_FIX",
          pboot(PP[["DYN_ALS_GEDIMAX"]],PP[["DYN_S2_ANNUAL_FIX"]])),
    cbind(window=w,contrast="GEDIMAX - STATIC_ALS",
          pboot(PP[["DYN_ALS_GEDIMAX"]],PP[["STATIC_ALS"]])))
}
...
fwrite(res,file.path(TAB,"Table18c_c4_gedimax.csv"))
fwrite(bt,file.path(TAB,"Table18d_c4_gedimax_boot.csv"))
