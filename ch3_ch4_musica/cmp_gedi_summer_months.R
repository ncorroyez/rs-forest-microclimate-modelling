# ==============================================================================
# cmp_gedi_summer_months.R — Chapter 4, "et l'été ?": month-by-month summer
...
# Writes NC_Full/manuscripts/ch4/tables/Table16_c4_summer_months.csv (+ _boot).
#   Rscript cmp_gedi_summer_months.R
# ==============================================================================
...
WINDOWS<-list(jun=seq(as.Date("2021-06-01"),as.Date("2021-06-30"),by="day"),
...
SC<-c("DYN_ALS_GEDIFALL","DYN_S2_ANNUAL_FIX","CONST_ALS","STATIC_ALS")
...
for(w in names(WINDOWS)){
  ...
  boot[[w]]<-rbind(
    cbind(window=w,contrast="RAMP: ANNUAL_FIX - CONST",
          pboot(PP[["DYN_S2_ANNUAL_FIX"]],PP[["CONST_ALS"]])),
    cbind(window=w,contrast="GEDIFALL - ANNUAL_FIX",
          pboot(PP[["DYN_ALS_GEDIFALL"]],PP[["DYN_S2_ANNUAL_FIX"]])))
}
...
fwrite(res,file.path(TAB,"Table16_c4_summer_months.csv"))
fwrite(bt,file.path(TAB,"Table16b_c4_summer_months_boot.csv"))
