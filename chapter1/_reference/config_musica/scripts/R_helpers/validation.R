...
build_hobo_inputs <- function(hobo_geojson, r_stack, ids_to_remove,
  ...
  if (hobo_buffer_mode == "buffer25") {
    ...
    buf_pts <- sf::st_buffer(hobo_pts, 25)
    buf_v   <- terra::vect(buf_pts)
    # PAD → LAI conversion factor (same as in load_lidar_rasters)
    lai_buf    <- 2 * terra::extract(r_lai_raw,    buf_v, fun = "mean", na.rm = TRUE)[, 2]
    ...
    lad_buf    <- 2 * terra::extract(r_lad_raw,    buf_v, fun = "mean", na.rm = TRUE)
...
validate_scenarios_at_hobos <- function(df_hobo_inputs, df_hobo_daily, scenarios, parent_dir, df_macro, date_seq, forcing_file, musica_cmd, force = FALSE) {
  ...
  for (nm in names(scenarios)) {
    ...
    for (i in seq_len(nrow(df_hobo_inputs))) {
      ...
      run_musica_one(plot_row, sc, out_file, forcing_file, musica_cmd)
    }
    hobo_paths[nm] <- out_d
  }
