...
# Out: out_files/Chapter3/tables/metrics6_<ver>/part_<chunk>.csv (long: per metric)
# ==============================================================================
...
ncp <- function(pid, tag) {
  if (VER == "iter") return(file.path("out_files/Chapter3/nc_sensitivity_perplot_iter", sprintf("%s_%s.nc", pid, tag)))
  # v320: base/unif from the Shapley lattice (1111/1110), perturbations from nc_sensitivity_perplot
  if (tag == "base")    return(file.path("out_files/Chapter3/nc_shapley2x", sprintf("%s_1111.nc", pid)))
  if (tag == "unifLAD") return(file.path("out_files/Chapter3/nc_shapley2x", sprintf("%s_1110.nc", pid)))
  file.path("out_files/Chapter3/nc_sensitivity_perplot", sprintf("%s_%s.nc", pid, tag))
}
...
pdir <- sprintf("out_files/Chapter3/tables/metrics6_%s", VER); dir.create(pdir, recursive=TRUE, showWarnings=FALSE)
