...
each profile is renormalised to preserve leaf area. We crossed μ from 0.30 to 0.70 with σ from 0.12 to 0.36,
at five one-sided leaf-area levels (LAI 2 to 6). Cover was fixed at 0.87 and maximum height at 25 m. MuSICA
...
Leaf quantity dominated from the intermediate to the closed archetypes and strengthened towards the closed
end (Fig. 4, Table 2). In the densest archetype P4, adding 0.5 of leaf area cooled the understorey by 0.205 °C
...
largest scalar sensitivities, and the leaf-area effect still strengthened with density. The profile's rank against the leaf-area step differed on the slope. Its contrast was +0.002 in P1, −0.002 in P2, +0.008 in P3 and +0.027 in P4
...
leaf-area effect seen here reflects a deciduous signature or the parameterisation of this canopy remains to
be tested in stands of other leaf habit and density.
...
recover a large share of the microclimate signal a LiDAR model captures
...
Appendix D. That shift alone changes the simulated coupling (Appendix D); the companion forcing study assesses it against
sub-canopy observations (Corroyez et al., 2026a, in preparation). An optical proxy should therefore keep its
leaf-area error below 23%, and optical indices tend to saturate in dense canopy before reaching such accuracy.
...
before it can (Corroyez et al., 2026b, in preparation).
...
of eight plots of the CHS 41 configuration. Per unit of leaf area, the leaf-area effect is −0.42 °C in P4
...
excess comes from aircraft roll. Their median magnitude is 16° and the 95th percentile 29°. This gives
...
(Appendix A), it leaves the variable ranking unchanged. We verified this by re-running the attribution. That is
expected: a near-uniform 5% rescaling moves every plot's leaf area by the same fraction, so it shifts the
absolute magnitudes without reordering the sensitivities. A larger rescaling can change the coupling,
as the extinction-coefficient test next shows.

![**Figure D1** Scan-angle correction of leaf-area density, characterised on a 53-plot ALS sample at Blois and applied to all 400 design plots. **(a)** Mean leaf-area-density profile from the standard lidR inversion (grey) and after the scan-angle correction (red dashed). The corrected profile lies slightly below the lidR profile at every height, a leaf-area reduction of about 5%. **(b)** Per-plot ⟨sec θ⟩ factor: all values lie between 1.02 and 1.09.](figures/article_v323/FigD1_scanangle_control.png){width=92%}

We also tested the extinction coefficient itself. An optimised value *k* = 0.65 was obtained for this ALS
product in our companion Sentinel-2/LiDAR retrieval study (Corroyez et al., 2026b, in preparation). It
...
different objective, the inter-sensor consistency of the leaf area. The consequence of
...
to transfer. At a site whose true leaf area departs from the *k* = 0.5 proxy the scheme could fail. We claim
that only the variable ranking, which the near-uniform corrections leave intact, transfers beyond this
dataset.

# Appendix E. Uncertainty of the variable effects

Each plot contributes both directions of every scalar step, so the sign and the curvature of an effect are
read from the data rather than assumed. Table 2 of the main text gives the ΔT~max~ effects with their
intervals. Table E1 gives the same design on the buffering slope and shows which of those patterns depend
on the response metric. Maximum height stays at or below 0.001 per metre, and leaf area and cover keep
both their sign and their strengthening towards the dense end, so the three scalar sensitivities transfer
to the slope. The profile contrast is small in the open and intermediate archetypes (+0.002 in P1, −0.002
in P2, +0.008 in P3) and largest in the dense P4 (+0.027). In P3 it matches the leaf-area step (−0.008) and
in P4 it exceeds it (−0.022), with the opposite sign. It stays at or below the cover step in both
(−0.012 and −0.030). Its positive sign in P3 and P4 means the real profile weakens buffering relative to the
uniform one.
...
[@ogeeMuSICACO2Water2003], using the official binary released under the EUPL. The model runs for a
...
heterogeneity [@garrattReviewAtmosphericBoundary1994]; it is conventionally a small fraction of the
boundary-layer height. The model takes that blending height as a prescribed input and does not derive it
...
median is 550 m. @bouwenInteractionsEntreStructure instead derived it from ERA5, following the reanalysis
boundary-layer-height climatology of @seidelClimatologyPlanetaryBoundary2012.

The soil is resolved on an irregular grid to 1.6 m depth with van Genuchten retention, and phenology
...
contrast), not a parametric default.
...
| Exponent *m* | 0.5 (all depths), a namelist value the model does not read under this flag combination: the retention model fixes the retention exponent internally to 1, and *m* enters only the Brooks and Corey conductivity branch, which this configuration does not select |
...
The main figures are computed over all summer days. Because forest microclimatic buffering matters most
during heat extremes, we repeated the analyses over the hottest 10% of days (13 days, ranked by the station
daily maximum). The result is unchanged: leaf quantity controls the buffering on hot days as
over the full summer. Most effects are larger under heat, and so is their spread. The per-plot dispersion of
the leaf-area effect widens by a factor of about 1.4 to 1.7 relative to the full summer, so the hot-day
medians are read as a direction, not as precise estimates.

![**Figure S1** Variable attribution as in Figure 4 over the hottest 10% of days. Leaf quantity remains dominant in the dense P4 (leaf area +0.5 giving −0.268 °C, cover +10 points −0.284 °C). The full-contrast profile increases in the intermediate archetypes P2 and P3 (−0.078 and −0.121 °C, against −0.052 and −0.065 °C over the full summer), and maximum height stays negligible.](figures/FigS1_attribution_hot_chs41.png){width=90%}
