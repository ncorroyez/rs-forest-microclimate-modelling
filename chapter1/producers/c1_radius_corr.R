# ==============================================================================
# Radius sensitivity on the CORRECTED baseline (station forcing + wind correction
# + scan-angle-corrected LAI + v3.2.3 iter), so it matches the reported model.
# Per HOBO plot: clip_circle(radius) -> LAD/LAI/Hmax/fCover + <sec theta> -> MuSICA.
...
OUT<-file.path("out_files/radius_test_corr",paste0(radius,"m")); dir.create(OUT,recursive=TRUE,showWarnings=FALSE)
