# ==============================================================================
# FPC-typology re-run — per-unit sensitivity on the NEW {LAI,Hmax,fCover,FPC1-3}
# cLHS sample (clhs_sample_fpc_v1.rds), SAME legacy v3.2.0 binary + ERA5 forcing
# as the frozen run (so ONLY the typology differs → apples-to-apples vs metrics6_v320).
# ISOLATED: writes nc_sensitivity_perplot_fpc/ + sensitivity_perplot_fpc/. Resumable.
#   Rscript c1_sensitivity_perplot_chunk_fpc.R <chunk> <K>
# ==============================================================================
...
CFG_C3$musica_cmd <- "/home/corroyez/Documents/musica/musica"          # validated legacy v3.2.0 (md5 5307)
stopifnot(file.exists(CFG_C3$musica_cmd))

samp <- as.data.table(readRDS("out_files/Sensitivity_Analysis/clhs_sample_fpc_v1.rds"))   # NEW FPC sample
...
cat(sprintf("FPC chunk %d/%d : %d plots\n", chunk, K, length(my_rows)))
...
NCDIR <- "out_files/Chapter1/nc_sensitivity_perplot_fpc"; dir.create(NCDIR, recursive = TRUE, showWarnings = FALSE)
...
run_get <- function(prow, tag, lai, hmax, fcov, ladf = make_lad_real) {
  ...
  sc <- list(lai_fn = function(p) lai, hmax_fn = function(p) hmax, fcover_fn = function(p) fcov,
             lad_fn = ladf, phenology_fn = NULL)
  run_musica_one(prow, sc, nc, CFG_C3$forcing_file, CFG_C3$musica_cmd)
  if (!file.exists(nc) || file.size(nc) < 1000) return(NA_real_)
  metrics_one(nc)
}

pdir <- "out_files/Chapter1/tables/sensitivity_perplot_fpc"; dir.create(pdir, showWarnings = FALSE, recursive = TRUE)
...
for (j in seq_along(my_rows)) {
  ...
  r <- tryCatch({
    ...
    data.table(pid = prow$pid, Cluster = prow$Cluster, P = prow$P, LAI = prow$LAI, Hmax = prow$Hmax, fCover = prow$fCover, VCI = prow$VCI,
               base = base,
  ...
  if (j %% 5 == 0) cat(sprintf("  FPC chunk %d: %d/%d\n", chunk, j, length(my_rows)))
}
cat(sprintf("FPC chunk %d DONE (%d/%d rows)\n", chunk, nok, length(my_rows)))
