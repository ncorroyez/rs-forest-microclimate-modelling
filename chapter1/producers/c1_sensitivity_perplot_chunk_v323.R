# ==============================================================================
# v3.2.3 counterpart of c1_sensitivity_perplot_chunk.R — the ARTICLE per-unit
# sensitivity (ΔLAI/ΔHmax/ΔfCover + LAD real-vs-uniform) run with MuSICA v3.2.3
# instead of legacy v3.2.0, on the SAME 400 cLHS plots, so Fig 2/3 importance can
# be compared across versions. Nothing for v3.2.3 exists on the 400 cLHS, so every
# sim is fresh: base (real) + 6 scalar perturbations + uniform-LAD = 8 sims/plot.
# Same pid mapping, same metric (fixed 1 m, -2 h, vs macro), resumable.
#   Rscript c1_sensitivity_perplot_chunk_v323.R <chunk> <K>
# Out: out_files/Chapter1/tables/sensitivity_perplot_v323/part_<chunk>.csv
# ==============================================================================
...
CFG_C3$musica_cmd <- normalizePath("in_files/model-3.2.3/musica", mustWork = TRUE)  # v3.2.3
...
cat(sprintf("v323 chunk %d/%d : %d plots\n", chunk, K, length(my_rows)))
...
NCDIR <- "out_files/Chapter1/nc_sensitivity_perplot_v323"; dir.create(NCDIR, recursive = TRUE, showWarnings = FALSE)
...
pdir <- "out_files/Chapter1/tables/sensitivity_perplot_v323"; dir.create(pdir, showWarnings = FALSE, recursive = TRUE)
...
for (j in seq_along(my_rows)) {
  ...
  if (j %% 5 == 0) cat(sprintf("  v323 chunk %d: %d/%d\n", chunk, j, length(my_rows)))
}
cat(sprintf("v323 chunk %d DONE (%d/%d rows)\n", chunk, nok, length(my_rows)))
