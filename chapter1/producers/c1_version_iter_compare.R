...
L <- load_parts("out_files/Chapter1/tables/sensitivity_perplot")
L <- merge(L, fread("out_files/Chapter1/tables/tab_sensitivity_perplot_lad.csv")[,.(pid,dT_LAD)], by="pid", all.x=TRUE)
imp <- rbind(
  imp_from(L)[, version:="v3.2.0"],
  imp_from(load_parts("out_files/Chapter1/tables/sensitivity_perplot_v323"))[, version:="v3.2.3 none"],
  imp_from(load_parts("out_files/Chapter1/tables/sensitivity_perplot_iter"))[, version:="v3.2.3 iter (realBLH)"]
)
fwrite(imp, "out_files/Chapter1/tables/tab_version_iter_compare.csv")
...
ggsave("out_files/Chapter1/figures/FigSh_version_iter_compare.png", p, width=12, height=5, dpi=200, bg="white")
