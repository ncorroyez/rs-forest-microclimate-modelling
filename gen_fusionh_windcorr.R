# ==============================================================================
# FUSION_H (layered §3.5) re-run WITH per-plot wind correction, so the fusion bar
...
# scaled per plot. Writes out_files/Chapter3/nc_genuine53_windcorr/FUSION_H/.
...
NCROOT <- file.path(CFG_C3$out_dir, "nc_genuine53_windcorr"); dir.create(file.path(NCROOT,"FUSION_H"), recursive=TRUE, showWarnings=FALSE)
...
# ---- FUSION_H recipe (identical to run_v323iter_ch3_extra.R, masked basis) -----
...
SC <- wrap_iter(list(name="FUSION_H", lai_fn=.fn("LAI_ALS"), hmax_fn=.fn("Hmax"),
...
run_one <- function(i){ prow<-as.data.frame(df[i,])
  nc<-file.path(NCROOT,"FUSION_H",sprintf("musica_out_HOBO_%s.nc",prow$id_plot))
...
cat(sprintf("FUSION_H windcorr: %d plots on %d cores\n", nrow(df), NCORES))
