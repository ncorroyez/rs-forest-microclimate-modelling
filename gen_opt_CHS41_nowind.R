...
NCROOT <- if(DRY) file.path(CFG_C3$out_dir,"nc_DRY") else file.path(CFG_C3$out_dir,"nc")
dir.create(file.path(NCROOT,"STATIC_S2_OPT"), recursive=TRUE, showWarnings=FALSE)
WFDIR  <- file.path(CFG_C3$out_dir, "windcorr_forc_chs41"); dir.create(WFDIR, recursive=TRUE, showWarnings=FALSE)

# NO wind correction: Chapter 3 runs on the CHS41 forcing as-is, matching the
# other scenarios in out_files/Chapter3_CHS41/nc. The per-plot log-profile wind
# scaling is kept out of the chapter and illustrated only in Chapter 1.
windcorr_pblh <- function(hmax) FORC_BASE
