...
# ---- data: GENUINE 53/53 magnitudes (no fabrication), built by build_real53_df.R
prep <- load_lai_prep(CFG_C3); tb <- prep$ts_by_plot; dopt <- CFG_C3$d_opt_m
df <- readRDS(file.path(CFG_C3$out_dir, "lai_prep", "df_plots_real53.rds"))
stopifnot(sum(is.na(df$LAI_ALS_DOPT))==0, sum(is.na(df$LAI_S2_DOPT))==0, sum(is.na(df$LAI_S2_RESCALED))==0)
...
NCROOT <- file.path(CFG_C3$out_dir, "nc_real53")
