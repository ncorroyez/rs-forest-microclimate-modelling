...
# effect is isolated. 3 reference scenarios x 53 plots -> nc_genuine53_windcorr_dry.
...
SCEN <- c("CONST_ALS","STATIC_ALS","DYN_S2_ATBD")   # reference scenarios
NCROOT <- file.path(CFG_C3$out_dir, "nc_genuine53_windcorr_dry")
WFDIR  <- file.path(CFG_C3$out_dir, "windcorr_forc_pblh")   # cache of wind-scaled pblh forcings
...
cat(sprintf("WINDCORR dry: %d runs (%d scen x 53) on %d cores\n", nrow(tasks), length(SC), NCORES))
