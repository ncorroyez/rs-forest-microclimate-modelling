...
full_heights <- data.frame(height = seq(2.5, 39.5, by = 1))
all_allometry_list <- list()
...
for (radius in radius_list) {
  ...
  for (forcing_type in forcing_types) {
    for (i in seq_len(length(n_field))) {
      ...
      complete_allometry <- merge(full_heights, allometry, by = "height", all.x = TRUE)
      complete_allometry$density[is.na(complete_allometry$density)] <- 0
      
      # Prepare data for CSV storage (wide format recommended for profiles)
      # We add metadata to identify the plot and radius
      storage_df <- data.frame(
        id_plot = n_field[i],
        radius = radius,
        t(complete_allometry$density) # Transpose densities to columns
      )
      
      # Rename density columns based on height for clarity
      colnames(storage_df)[3:ncol(storage_df)] <- paste0("h_", complete_allometry$height)
      
      # Store in the global list
      all_allometry_list[[length(all_allometry_list) + 1]] <- storage_df
      ...
      # })
      # stop()
      history_file <- paste0("./out_files/musica_out_", forcing_type,
...
final_allometry_df <- do.call(rbind, all_allometry_list)
write.csv(final_allometry_df, "allometry_profiles.csv", row.names = FALSE)
