# ==============================================================================
# Chapter 3 — Table 4f, COMPLETE 6-scenario set under v3.2.3-iter, at 53 plots
# each (fair). Reads out_files/Chapter1/nc_v323iter/ (the only v3.2.3 cache that
# holds the _NM + opt-LUT scenarios regen_table4f.R was written for). Summer 2021,
# NO shift, 1 m. Identical logic to regen_table4f.R; only ncp + output differ.
# Cross-check: LiDAR full here must match the Chapter3/nc/ 2021 extraction
# (dT_r2 0.346 / sl_r2 0.597) -> confirms the two v3.2.3 caches agree on summer.
# Run from z_Example root:  Rscript regen_table4f_v323full.R
# ==============================================================================
...
ncp <- "out_files/Chapter1/nc_v323iter"                 # v3.2.3 _NM cache (2-yr forcing)
scen <- c(DYN_S2_ATBD_NM="S2 ATBD (dyn)", DYN_S2_OPT_NM="S2 opt (dyn)",
          DYN_ATBD_rfull_NM="S2 ATBD x ratio->full", DYN_OPT_rdopt_NM="S2 opt x ratio->d_opt",
...
cat(sprintf("\nbinary = v3.2.3-iter | cache = %s | macro forcing = %s\n", ncp, CFG_C3$forcing_file))
cat(sprintf("hottest-days threshold (macro Tmax p90) = %.2f C (%d days)\n", thr, length(hot)))
cat("\n=== Table 4f v3.2.3 COMPLETE (NO SHIFT, 1 m, summer) ===\n")
print(R[order(period,-dT_r2), .(scenario,period,n,dT_r2,dT_rmse,dT_bias,sl_r2,sl_rmse,sl_bias)])
cat("\nCROSS-CHECK LiDAR full summer 'all' should ~= 0.346 / 0.597 (Chapter3/nc 2021 cache)\n")
cat("DONE -> Table4f_summer_2metrics_2periods_v323.csv (complete 6-scenario, 53 plots)\n")
