...
# per-plot factor: rescale S2 shape so summer level = LiDAR LAI_ALS  (LiDAR d_opt magnitude × S2 timing)
fp <- setNames(df$LAI_ALS_DOPT / df$LAI_S2_ATBD, df$pid)
...
ts_alsmag <- setNames(lapply(seq_len(nrow(df)), function(i){ p<-df$pid[i]; a<-atbd[[p]]
  if(is.null(a)) return(data.frame(doy=1:365, lai=rep(df$LAI_ALS_DOPT[i],365)))
  data.frame(doy=a$doy, lai=a$lai*fp[[p]]) }), df$pid)

sc <- list(DYN_ALSDOPT_S2TIMING_NM = list(name="DYN_ALSDOPT_S2TIMING_NM",
  lai_fn=.fn("LAI_ALS_DOPT"), hmax_fn=.fn("Hmax"), fcover_fn=.fn("fCover"),
...
cat("\n=== MuSICA done for DYN_ALSDOPT_S2TIMING_NM; computing monthly slope R2 ===\n")
...
fs <- list.files(file.path(ncp,"DYN_ALSDOPT_S2TIMING_NM"), pattern="\\.nc$", full.names=TRUE)
...
cat("\n=== DYN_ALSDOPT_S2TIMING_NM (LiDAR d_opt magnitude × S2 timing) — slope R2 by month ===\n")
print(res)
cat("\nCOMPARE April slope R2:  LiDAR full 0.092 | this 0.??? | S2 ATBD 0.508\n")
fwrite(res, "/home/corroyez/Documents/NC_Full/manuscripts/ch3/tables/Table4u_decouple_alsdopt_s2timing.csv")
