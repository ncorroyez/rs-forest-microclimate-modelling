# ==============================================================================
# Per-plot AVG fig — variant where x/y axes use log(slope) (micro vs macro)
# instead of ΔTmax. Same 5×5 grid, same per-cluster Δv-descending order
# (derived from heatmap Tmax-based LOO).
#
# Convention :
#   slope = lm(Tair_micro_hourly ~ Tair_macro_ERA5_hourly)[2]
#   log_slope = log(slope) ; one value per (plot, coalition)
#   log_slope < 0 → buffer, log_slope > 0 → amplification (legacy main_compare_era5_ign.R)
#
# Output : fig_forward_HOBO_v10_fixed_per_cluster_plus_all_AVG_logslope.{png,pdf}
# ==============================================================================
...
# ---- Δv per cluster (FROM heatmap Tmax, same as AVG ΔTmax version) ---------
...
# Build bit sequence per cluster with plotmath labels
...
p <- ggplot(long, aes(x = log_slope_obs, y = log_slope_sim,
  ...
  # 0 = slope=1 separator between buffer (neg) and amp (pos)
  ...
  geom_text(data = ann, aes(x = Inf, y = -Inf, label = panel_lab),
             ...
             size = 4.2, fontface = "italic", parse = TRUE,
...
ggsave(file.path(OUT, "fig_forward_HOBO_v10_fixed_per_cluster_plus_all_AVG_logslope.png"),
        p, width = 24, height = 13.5, dpi = 600, bg = "white")
ggsave(file.path(OUT, "fig_forward_HOBO_v10_fixed_per_cluster_plus_all_AVG_logslope.pdf"),
        p, width = 24, height = 13.5, device = cairo_pdf)
cli_alert_success("Saved fig_forward_HOBO_v10_fixed_per_cluster_plus_all_AVG_logslope (png + pdf, 16:9)")

fwrite(ann[, .(Cluster_row, step, label,
                r = round(r_val, 3), RMSE = round(rmse_val, 4),
                MAE = round(mae_val, 4), n = n_val)],
        file.path(OUT, "tab_v10_HOBO_fixed_per_cluster_plus_all_AVG_logslope.csv"))
