# ==============================================================================
# Per-plot AVG fig — V11 (model-3.2.3), log(slope), subset version.
# Mirror of make_fixed_per_cluster_avg_logslope_subset.R but reads V11 outputs
# from out_files/musica_hobo_v11_model323/ and H1_archetypes_v11_model323/.
# Output : fig_forward_HOBO_v11_subset_<rows>_AVG_logslope.{png,pdf}
# ==============================================================================
...
# HOBO observed log slope (UTC, no shift)
...
# V11 simulated log slope per plot per coalition
collect_step_slope <- function(bit) {
  subdir <- file.path("out_files/musica_hobo_v11_model323", bit)
  ...
  for (f in nc_files) {
    ...
    rows[[id]] <- data.table(id_plot = id, slope_sim = res$slope,
                              log_slope_sim = log(abs(res$slope)))
  }
  rbindlist(rows)
}

cli_h1("V11 — Pre-computing 16 HOBO coalitions (log slope)")
PP <- list()
...
# Δv per cluster from V11 archetypes (Tmax-based, same convention as V10 fig)
...
cli_alert("Subset : {paste(clusters, collapse=' + ')}")
for (cl in clusters) cli_alert("{cl} order : {paste(order_per_cluster[[cl]], collapse=' -> ')}")
...
p <- ggplot(long, aes(x = log_slope_obs, y = log_slope_sim,
  ...
  labs(x = expression("Field " ~ log(slope[micro/macro])),
       y = expression("Simulated " ~ log(slope[micro/macro]) ~ "(v11)")) +
...
ggsave(file.path(OUT, sprintf("fig_forward_HOBO_v11_subset_%s_AVG_logslope.png", suffix)),
        p, width = fig_w, height = fig_h, dpi = 600, bg = "white", limitsize = FALSE)
ggsave(file.path(OUT, sprintf("fig_forward_HOBO_v11_subset_%s_AVG_logslope.pdf", suffix)),
        p, width = fig_w, height = fig_h, device = cairo_pdf, limitsize = FALSE)
cli_alert_success("Saved fig_forward_HOBO_v11_subset_{suffix}_AVG_logslope (png + pdf)")

fwrite(ann[, .(Cluster_row, step, label,
                r = round(r_val, 3), RMSE = round(rmse_val, 4),
                MAE = round(mae_val, 4), n = n_val)],
        file.path(OUT, sprintf("tab_v11_HOBO_subset_%s_AVG_logslope.csv", suffix)))
