# ==============================================================================
# Heatmap LOO on hourly log(slope), V10 archetypes (per cluster + Avg).
#
# Convention :
#   slope = lm(Tair_micro_hourly ~ Tair_macro_ERA5_hourly)[2]
#   metric = log(|slope|) — legacy main_compare_era5_ign.R
#   Δv = log_slope(REF) − log_slope(LOO_v) ; negative = LOO_v removes buffering
#                                            (variable is a buffer driver).
#   MuSICA UTC ← shifted -2 h inside extract_hourly_slope_one to align with ERA5.
#
# Output : fig_heatmap_LOO_v10_logslope.{png,pdf}
# ==============================================================================
...
arch_nc_path <- function(cl, bit) {
  ...
  v10_path <- file.path("out_files/H1_archetypes_v10_fcovmean", arch_lbl,
                        sprintf("musica_out_%s_%s.nc", arch_lbl, sc_name_of[bit]))
  v9_path  <- file.path("out_files/H1_archetypes_v9", arch_lbl,
                        sprintf("musica_out_%s_%s.nc", arch_lbl, sc_name_of[bit]))
...
cli_h1("Building V10 wide table on log(slope)")
rows <- list()
...
ggsave(file.path(OUT, "fig_heatmap_LOO_v10_logslope.png"),
        p, width = 16, height = 9, dpi = 600, bg = "white")
ggsave(file.path(OUT, "fig_heatmap_LOO_v10_logslope.pdf"),
        p, width = 16, height = 9, device = cairo_pdf)
cli_alert_success("Saved fig_heatmap_LOO_v10_logslope (png + pdf, 16:9)")

# CSV table
fwrite(long_full, file.path(OUT, "tab_heatmap_LOO_v10_logslope.csv"))
