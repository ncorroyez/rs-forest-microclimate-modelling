# ==============================================================================
# Hourly MuSICA (y) vs HOBO (x), REF coalition (1111), legacy convention :
#   - Tair extracted at nair == 1 (lowest air node, NOT z=1m interpolation)
#   - No -2h time shift (UTC ↔ UTC)
# Reproduces main_compare_era5_ign.R / Silvilaser convention that gave the
# 45 buffering / 8 amplifying split.
#
# Two figs :
#   - fig_hourly_sim_vs_obs_REF.{png,pdf}      all 53 plots
#   - fig_hourly_sim_vs_obs_REF_buf.{png,pdf}  buffering plots only (slope < 1)
# ==============================================================================
...
cli_h1("Hourly MuSICA vs HOBO (REF, nair==1, no shift)")

# 1. HOBO hourly observations (z=1m sensor, position_sensor 'a')
...
# 2. MuSICA REF predictions at nair == 1 (legacy convention, no time shift)
...
cli_alert("MuSICA hourly rows : {nrow(DT_sim)} | {length(unique(DT_sim$id_plot))} plots")

# 3. Merge by (plot, hour)
...
# 4. Per-plot slope (Tair_sim ~ Tair_era5 via merge later; here for buf/amp
#    classification use Tair_sim ~ Tair_obs — but legacy uses ERA5. Use ERA5.)
...
cli_alert("Buffering : {length(buf_ids)} | Amplifying : {length(amp_ids)}")
...
make_fig <- function(dt, stats, png_path, subtitle) {
  ...
  p <- ggplot(dt, aes(x = Tair_obs, y = Tair_sim)) +
    ...
    labs(x = expression("Field hourly " * italic(T)[air] ~ "(HOBO, °C)"),
         y = expression("Simulated hourly " * italic(T)[air] ~ "(MuSICA REF, °C)"),
         subtitle = subtitle) +
...
cli_h2("ALL 53 plots")
cli_alert("r = {round(st_all$r, 4)}   R² = {round(st_all$r2, 4)}   RMSE = {round(st_all$rmse, 3)} °C   MAE = {round(st_all$mae, 3)} °C   bias = {round(st_all$bias, 3)} °C")
make_fig(DT, st_all,
          file.path(OUT, "fig_hourly_sim_vs_obs_REF.png"),
          sprintf("All %d HOBO plots (nair = 1, UTC ↔ UTC)", length(unique(DT$id_plot))))

# 8. Buffering subset only (slope < 1, legacy convention)
DT_buf <- DT[id_plot %in% buf_ids]
st_buf <- stats_block(DT_buf)
cli_h2("Buffering plots only (slope < 1, n = {length(buf_ids)})")
cli_alert("r = {round(st_buf$r, 4)}   R² = {round(st_buf$r2, 4)}   RMSE = {round(st_buf$rmse, 3)} °C   MAE = {round(st_buf$mae, 3)} °C   bias = {round(st_buf$bias, 3)} °C")
make_fig(DT_buf, st_buf,
          file.path(OUT, "fig_hourly_sim_vs_obs_REF_buf.png"),
          sprintf("Buffering plots only (slope < 1, n=%d)", length(buf_ids)))

# 9. Amplifying subset
DT_amp <- DT[id_plot %in% amp_ids]
if (nrow(DT_amp) > 0) {
  st_amp <- stats_block(DT_amp)
  cli_h2("Amplifying plots only (slope > 1, n = {length(amp_ids)})")
  cli_alert("r = {round(st_amp$r, 4)}   R² = {round(st_amp$r2, 4)}   RMSE = {round(st_amp$rmse, 3)} °C   MAE = {round(st_amp$mae, 3)} °C   bias = {round(st_amp$bias, 3)} °C")
  make_fig(DT_amp, st_amp,
            file.path(OUT, "fig_hourly_sim_vs_obs_REF_amp.png"),
            sprintf("Amplifying plots only (slope > 1, n=%d)", length(amp_ids)))
}

# Save stats CSV
fwrite(rbind(
  data.table(subset = "all", n_plots = length(unique(DT$id_plot)),
              r = st_all$r, R2 = st_all$r2, RMSE = st_all$rmse,
              MAE = st_all$mae, bias = st_all$bias, n_hourly = st_all$n),
  data.table(subset = "buffering", n_plots = length(buf_ids),
              r = st_buf$r, R2 = st_buf$r2, RMSE = st_buf$rmse,
              MAE = st_buf$mae, bias = st_buf$bias, n_hourly = st_buf$n)
), file.path(OUT, "tab_hourly_sim_vs_obs_REF_stats.csv"))
