...
p_norm <- base_layers(
  ...
  labs(x = NULL, y = "Relative height, z / canopy top",
       caption = paste0("JJAS means, 10:00–16:00 on the forcing clock ",
                        "(solar time, UTC+1; 11:00–17:00 local summer time, UTC+2)"))
...
if (length(.vf)) {
  ...
  cat(sprintf("\n1 m sub-canopy diurnal peak: %02d:00 on the forcing clock (solar time, UTC+1), i.e. %02d:00 local summer time UTC+2 (%.2f degC); window used %02d-%02d forcing clock\n",
              .c[which.max(T), h], .c[which.max(T), h] + 1L, .c[, max(T)], min(HOURS), max(HOURS)))
