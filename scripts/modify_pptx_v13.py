"""
Corroyez_MEB2026_v12.pptx → v13.pptx.

Replace the forward-selection figure on slide 10 with the new version
(annotations : r, R², RMSE, MAE — Spearman ρ removed). Update slide 10 +
slide 8 notes to reflect the change : linear regression r/R²/RMSE/MAE
...
SRC = Path("/home/corroyez/Downloads/1Papers/files/Corroyez_MEB2026_v12.pptx")
DST = Path("/home/corroyez/Downloads/1Papers/files/Corroyez_MEB2026_v13.pptx")
...
# Slide 10 notes — drop Spearman, mention r, R², RMSE, MAE
set_notes(prs.slides[9], (
"From theory to data. For each of the 53 HOBO sensors we computed the "
"observed ΔTmax — Tmicro_obs minus Tmacro, on the x-axis — and the "
"simulated ΔTmax from MuSICA at each forward-selection step, on the "
"y-axis. Same convention on both axes: negative means the canopy buffers. "
"We start from the uniform baseline, then add traits one at a time in "
"the LOO order: fCover, LAI, Hmax, finally LAD = REF.\n"
"\n"
"Reading: the Pearson r improves from −0.4 with the baseline to +0.53 "
"with full structure restored. The linear-regression R² peaks at 0.78 "
"after the first add, but drops at later steps — and the slope stays "
"well below 1. The error metrics (RMSE around 2.1 °C, MAE around 1.9 °C) "
"barely move across steps. This is the glass-ceiling: simulated buffering "
"spans about 2 °C while observed range is about 7 °C, so MuSICA gets "
"part of the ordering, not the magnitude.\n"
"\n"
...
# Slide 8 notes — replace 'Spearman ρ' wording
...
"simulated ΔTmax matches the observed one at each step. Linear regression "
"with Pearson r, R², RMSE and MAE.\n"
"\n"
