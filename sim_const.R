...
sc<-make_all_scenarios_c3(prep$ts_by_plot,list_year=CFG_C3$list_year,d_opt=CFG_C3$d_opt_m,mode=CFG_C3$scenarios_mode)["CONST_ALS"]
stopifnot(!is.null(sc$CONST_ALS)); cat("simulating CONST_ALS\n")
...
cat(sprintf("CONST_ALS nc: %d\nSIM_DONE\n",length(list.files(file.path(CFG_C3$out_dir,"nc","CONST_ALS"),pattern="\\.nc$"))))
