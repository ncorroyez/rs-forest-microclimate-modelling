# Simulate DYN_S2_DOPT_PURE into the common nc/ dir. SAFE sourcing (no h1_/lovb_).
...
new <- sc_all["DYN_S2_DOPT_PURE"]
stopifnot(!is.null(new$DYN_S2_DOPT_PURE))
...
cat(sprintf("DYN_S2_DOPT_PURE nc: %d\nSIM_DONE\n",
            length(list.files(file.path(CFG_C3$out_dir, "nc", "DYN_S2_DOPT_PURE"), pattern="\\.nc$"))))
