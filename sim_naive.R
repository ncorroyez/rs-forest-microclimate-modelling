...
sc<-make_all_scenarios_c3(prep$ts_by_plot,list_year=CFG_C3$list_year,d_opt=CFG_C3$d_opt_m,mode=CFG_C3$scenarios_mode)["NAIVE_S2_FORMSH"]
stopifnot(!is.null(sc$NAIVE_S2_FORMSH)); cat("simulating NAIVE_S2_FORMSH\n")
...
cat(sprintf("NAIVE_S2_FORMSH nc: %d\nSIM_DONE\n",length(list.files(file.path(CFG_C3$out_dir,"nc","NAIVE_S2_FORMSH"),pattern="\\.nc$"))))
