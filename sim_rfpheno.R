# Simulate DYN_RF_PHENO into the common nc/ dir. SAFE sourcing (no h1_/lovb_).
...
new <- sc_all["DYN_RF_PHENO"]
stopifnot(!is.null(new$DYN_RF_PHENO))
...
cat(sprintf("DYN_RF_PHENO nc: %d\nSIM_DONE\n",
            length(list.files(file.path(CFG_C3$out_dir, "nc", "DYN_RF_PHENO"), pattern="\\.nc$"))))
