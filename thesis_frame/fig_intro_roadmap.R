# Figure — conceptual roadmap for the General Introduction.
...
draw_figure <- function() {
  ...
  rqs <- list(
    ...
    c("RQ3",
      "Which remotely sensed structure should force the model, and does the answer depend on the canopy archetype?")
  )
  chaps <- list(
    c("Chapter 1",
      "Controlled trait perturbation in MuSICA around each plot's own measured canopy, over 400 cLHS design plots.  ALS only, plot scale."),
    c("Chapter 2",
    ...
    c("Chapter 3",
      "Four leaf-area scenarios forced into MuSICA, scored on two buffering metrics against the sub-canopy plots, read archetype by archetype.")
  )
